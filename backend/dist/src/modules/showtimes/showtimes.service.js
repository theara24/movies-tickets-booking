...
let ShowtimesService = class ShowtimesService {
    ...
    async create(dto) {
        ...
        if (conflictingShowtime) {
            throw new common_1.ConflictException(`Hall is already occupied from ${conflictingShowtime.startTime.toISOString()} to ${conflictingShowtime.endTime.toISOString()}`);
        }
    ...
    async getShowtimesByDate(date, cinemaId) {
        return this.findAll({ date, cinemaId, limit: 100 });
    }
    async getShowtimesByMovie(movieId, cinemaId, date) {

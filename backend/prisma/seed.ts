import { PrismaClient, UserRole, SeatType, MovieStatus } from '@prisma/client';
...
async function main() {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 10; c++) {
      ...
      if (seatNum === 40 || seatNum === 41) type = SeatType.COUPLE;
      if (seatNum === 1) type = SeatType.WHEELCHAIR;
  ...
  const foodItems = [
    { name: 'Large Popcorn', price: 5.50, category: 'POPCORN' as const, stock: 100 },
    { name: 'Medium Popcorn', price: 4.00, category: 'POPCORN' as const, stock: 100 },
    { name: 'Coca-Cola Large', price: 3.50, category: 'DRINKS' as const, stock: 200 },
    { name: 'Coca-Cola Medium', price: 2.50, category: 'DRINKS' as const, stock: 200 },
    { name: 'Nachos', price: 4.50, category: 'HOT_FOOD' as const, stock: 50 },
    { name: 'Hot Dog', price: 3.00, category: 'HOT_FOOD' as const, stock: 50 },
    { name: 'M&M Pack', price: 2.00, category: 'CANDY' as const, stock: 150 },
    { name: 'Combo #1 (Popcorn + Drink)', price: 7.00, category: 'COMBO' as const, stock: 100 },
  ];
